...
export function ContentOverlay({ item, containerStatus }: ContentOverlayProps) {
  // Hide overlay entirely when a game is active (HOT) so it doesn't
  // obstruct the game — only show the container status badge.
  const isActiveGame = item.type === 'GAME' && containerStatus === 'HOT';
  ...
  return (
    <>
      ...
      {/* Container status badge — always visible */}
